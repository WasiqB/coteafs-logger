...
import com.github.wasiqb.coteafs.logger.config.LoggerConfig;
...
class ConfigBuilder {
    ...
    private static String getAppenderPlugin (final LoggerType loggerType) {
        return switch (loggerType) {
            ...
            default -> "ReportPortalLog4j2Appender";
        };
    }

    private final LoggerConfig setting;

    ConfigBuilder () {
        super ();
        this.setting = DataSource.parse (LoggerConfig.class);
    }
    ...
    private void addAttributes (final AppenderComponentBuilder appenderCom, final Logger logSetting) {
        if (logSetting.getType () == CONSOLE) {
            ...
            appenderCom.addAttribute ("append", logSetting.isAppend ());
            appenderCom.addAttribute ("immediateFlush", logSetting.isImmediateFlush ());
        }
    }
